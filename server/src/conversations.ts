...
router.get("/conversations/search", (req: AuthRequest, res) => {
  ...
  ).all(ftsQuery, req.userId!) as Array<Record<string, unknown>>;

  // Also match conversation titles (not in FTS, small table — LIKE is fine)
  const titleRows = db.prepare(
    `SELECT id, title, updated_at, NULL as snippet
     FROM conversations
     WHERE user_id = ? AND title LIKE ? AND id NOT IN (${ftsRows.map(() => "?").join(",") || "''"})
     ORDER BY updated_at DESC LIMIT 10`,
  ).all(req.userId!, `%${q}%`, ...ftsRows.map((r) => r.id)) as Array<Record<string, unknown>>;
  ...
  res.json({
    results: results.map((r) => ({
      conversationId: r.id,
      title: r.title,
      snippet: (r.snippet as string | null)?.slice(0, 200) ?? null,
      updatedAt: r.updated_at,
    })),
  });
});
