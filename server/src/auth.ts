...
router.put("/auth/profile", authMiddleware, (req: AuthRequest, res) => {
  ...
  if (displayName !== undefined) {
    ...
    vals.push(String(displayName).slice(0, 20));
  }
  if (avatar !== undefined) {
    sets.push("avatar = ?");
    vals.push(avatar); // null or base64 string
  }
  if (systemPrompt !== undefined) {
    sets.push("system_prompt = ?");
    vals.push(String(systemPrompt).slice(0, 2000));
  }
  if (apiKey !== undefined) {
    sets.push("api_key = ?");
    vals.push(apiKey || null);
  }

  if (sets.length === 0) {
    res.status(400).json({ message: "未提供更新字段" });
    return;
  }
