...
router.post("/conversations/:id/share", authMiddleware, (req, res) => {
    ...
    db.prepare("INSERT OR REPLACE INTO share_tokens (token, conversation_id, expires_at) VALUES (?, ?, ?)").run(token, conv.id, expiresAt);
    res.json({ token, url: `/share/${token}`, expiresAt });
});
// Revoke share link
router.delete("/conversations/:id/share", authMiddleware, (req, res) => {
    const conv = db.prepare("SELECT id FROM conversations WHERE id = ? AND user_id = ?")
        .get(req.params.id, req.userId);
    if (!conv) {
        res.status(404).json({ message: "对话不存在" });
...
router.get("/share/:token", (req, res) => {
    const row = db.prepare("SELECT c.title, c.id FROM share_tokens s JOIN conversations c ON c.id = s.conversation_id WHERE s.token = ? AND s.expires_at > datetime('now')").get(req.params.token);
    if (!row) {
