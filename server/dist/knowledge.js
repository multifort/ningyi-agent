...
router.get("/knowledge/search", (req, res) => {
    ...
    const rows = db.prepare("SELECT id, name, substr(content, max(0, instr(lower(content), lower(?)) - 100), 300) as snippet FROM knowledge_base WHERE user_id = ? AND content LIKE ? LIMIT 10").all(q, req.userId, `%${q}%`);
    // Also search in conversations
    const convRows = db.prepare("SELECT c.id, c.title as name, substr(m.content, max(0, instr(lower(m.content), lower(?)) - 100), 300) as snippet FROM messages m JOIN conversations c ON c.id = m.conversation_id WHERE c.user_id = ? AND m.content LIKE ? LIMIT 5").all(q, req.userId, `%${q}%`);
    res.json({
        results: [
            ...rows.map(r => ({ id: r.id, name: r.name, snippet: r.snippet, source: "knowledge" })),
            ...convRows.map(r => ({ id: r.id, name: r.name, snippet: r.snippet, source: "conversation" })),
        ].slice(0, 15),
    });
});
